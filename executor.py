...
class Response:
    ...
    def error_message(self):
        if self.message is None:
            return "No response"
        if (
            self.message.packet.message_type == 0
            and self.message.packet.message_code == 0
        ):
    ...
    def legacy_repr(self):
        ...
        if self.success:
            return "Reply from {0}, {1} bytes in {2}ms".format(
                self.message.source, len(self.message.packet.raw), self.time_elapsed_ms
            )
        # Not successful, but with some code (e.g. destination unreachable)
        return "{0} from {1} in {2}ms".format(
            self.error_message, self.message.source, self.time_elapsed_ms
        )
    ...
    def __repr__(self):
        ...
        return 'status=ERR\tfrom={1}\terror="{0}"'.format(
            self.message.source, self.error_message
        )
...
class ResponseList:
    ...
    def append(self, value):
        ...
        if len(self) == 1:
            ...
            self.rtt_avg = (
                (self.rtt_avg * (len(self) - 1)) + value.time_elapsed
            ) / len(self)
    ...
    def __repr__(self):
        ...
        for response in self._responses:
            ret += "{0}\r\n".format(response)
        ret += "\r\n"
        ret += "Round Trip Times min/avg/max is {0}/{1}/{2} ms".format(
            self.rtt_min_ms, self.rtt_avg_ms, self.rtt_max_ms
        )
        return ret
...
class Communicator:
    ...
    def __init__(
        ...
        self.socket = network.Socket(
            target, "icmp", options=socket_options, source=source
        )
        ...
        while time_left > 0:
            ...
            if raw_packet != b"":
                ...
                if (
                    ...
                    if payload_matched:
                        return Response(
                            Message("", response, source_socket[0]),
                            timeout - time_left,
                            source_request,
        ...
        for payload in self.provider:
            ...
            if not match_payloads:
                self.responses.append(
                    self.listen_for(identifier, self.timeout, None, icmp_out)
                )
            else:
                self.responses.append(
                    self.listen_for(
                        identifier, self.timeout, icmp_out.payload, icmp_out
                    )
                )
